...
int MCTS(const ConnectFour& game, unordered_map<int, ConnectNode*>& tree, char myColour, int& currentId, int rootId, int rollouts, double greed) {
	...
	// will tell you how long the program runs for
	...
	int i = 0;
	while (i < rollouts) {
		// start current node as the root
	    currentNode = tree.find(root->id)->second;

	    // set the depth of this iteration to 0
	    depth = 0;
		...
		// keeps track of the next colour to be played based on currentNode
		char nextColour;
		...
		// check if there are no children and if so, start back propogation
		if (currentNode->children.empty()) {
			// add current depth to the depthAvg value
			depthAvg += depth;
			// determine if this is a winning node for the computer
			bool isWin = currentNode->currentGame.checkWin(currentNode->newMove);
			if (currentNode->playerTurn == false) {
				isWin = !isWin;
			}

			// complete backpropogation based on how well people do
			bool isDraw = currentNode->currentGame.checkDraw();
		    if (isDraw) {
				backPropogation(currentNode, tree, 0, rootId);
			} else if (isWin) {
				// greed will make him more likely to make poor moves to win
			...
			// start with a child node id and make that the current node
...
double rollout(ConnectNode* currentNode, char myColour, char playerColour, double& depth) {
	...
	// iterate until the game is finished
	while (!finished) {
		// play a random move
		int play = rand() % 7;
